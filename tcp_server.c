...
//One thread to handle one client each, connected to the server
void *client_handler(void *arg){
		...
		pthread_detach(pthread_self());		//Deyach thread upon completion
		return NULL;
}
...
void signal_handler(int sig){
		...
		if(head!=NULL){
				...
				send(fd_holder, leave_msg, strlen(leave_msg), 0);
		}
}
